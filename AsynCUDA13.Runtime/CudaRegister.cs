...
namespace AsynCUDA13.Runtime
{
    ...
    internal class CudaRegister : IRuntimeRegister, IDisposable
    {
        ...
        public readonly BindingList<CudaMem> AllocationsBindingList = [];

        /// <summary>
        /// Gets a read-only collection of all registered memory objects. This is a snapshot of the current state and does not reflect future changes.
        /// </summary>
        public IReadOnlyCollection<IRuntimeMem> Allocations => this.Memory.Values.ToList();
        ...
        public long TotalAllocatedBytes => this.Memory.Values.Sum(m => m.TotalSize);

        /// <summary>Gets the number of registered memory objects.</summary>
        public int AllocationCount => this.Memory.Count;
        ...
        /// <summary>
        /// Rebuilds the bindable <see cref="AllocationsBindingList"/> from the current memory objects,
        /// suppressing intermediate change notifications for efficiency.
        /// </summary>
        private void RefreshMemoryList()
        {
            lock (this.AllocationsBindingList)
            {
                this.AllocationsBindingList.RaiseListChangedEvents = false;
                this.AllocationsBindingList.Clear();
                foreach (var m in this.Memory.Values)
                {
                    this.AllocationsBindingList.Add(m);
                }
                this.AllocationsBindingList.RaiseListChangedEvents = true;
                this.AllocationsBindingList.ResetBindings();
            }
        }
        ...
        public IRuntimeMem? this[IntPtr indexPointer] => this.Memory.Values.FirstOrDefault(m => m.Pointers.Contains(indexPointer));
        ...
        public IRuntimeMem? this[string indexPointerOrId]
        {
            get
            {
        ...
        /// thread-affine, and this registry is used from an ASP.NET floatton where each request may run on a
        ...
        {
            IRuntimeMem? mem = this[indexPointer];
            if (mem == null)
            {
                return 0;
            }
        ...
        // Methods (Allocate)
        /// <summary>
        /// Allocates a float uninitialized device buffer and registers it.
        /// </summary>
        ...
        public IRuntimeMem? AllocateSingle<T>(IntPtr length) where T : unmanaged
        {
            if (length <= 0)
            {
                return null;
        ...
        /// <summary>
        /// Asynchronously allocates a float uninitialized device buffer and registers it.
        /// </summary>
        /// <typeparam name="T">The unmanaged element type to allocate.</typeparam>
        /// <param name="length">The number of elements to allocate.</param>
        /// <returns>A task producing the registered <see cref="CudaMem"/>, or <c>null</c> on failure.</returns>
        public async Task<IRuntimeMem?> AllocateSingleAsync<T>(IntPtr length) where T : unmanaged
        {
            if (length <= 0)
            {
                return null;
        ...
        public IRuntimeMem? PushData<T>(IEnumerable<T> data) where T : unmanaged
        {
            ...
            try
            {
                ...
                IntPtr length = (IntPtr) data.LongCount();
                CudaDeviceVariable<T> devVariable = new((long) length);
                var pointer = devVariable.DevicePointer;
        ...
        public IRuntimeMem? PushChunks<T>(IEnumerable<IEnumerable<T>> chunks) where T : unmanaged
        {
            ...
            try
            {
                ...
                IntPtr[] lengths = chunks.Select(chunk => (IntPtr) chunk.LongCount()).ToArray();
        ...
        /// </summary>
        /// <typeparam name="T">The unmanaged element type of the data.</typeparam>
        /// <param name="chunks">The host data arrays to upload.</param>
        /// <returns>A task producing the registered <see cref="CudaMem"/>, or <c>null</c> on failure.</returns>
        public async Task<IRuntimeMem?> PushChunksAsync<T>(IEnumerable<IEnumerable<T>> chunks) where T : unmanaged
        {
            return await this.PushChunksAsync((IEnumerable<IEnumerable<T>>) chunks);
        }

        // Methods (Pull)
        /// <summary>
        /// Copies the data of a float registered device buffer back to the host.
        /// </summary>
        /// <typeparam name="T">The unmanaged element type of the data.</typeparam>
        ...
        public List<T[]> PullChunks<T>(IntPtr indexPointer, bool keep = false) where T : unmanaged
        {
            ...
            try
            {
                ...
                {
                    T[] chunkData = new T[mem.PointerLengths[i]];
        ...
        /// <summary>
        /// Asynchronously copies the data of a float registered device buffer back to the host using a native async memcpy on a CUDA stream.
        /// </summary>
        /// <typeparam name="T">The unmanaged element type of the data.</typeparam>
        /// <param name="indexPointer">The native handle of a buffer belonging to the allocation to read.</param>
        /// <param name="keep">If <c>false</c>, the device memory is freed after the copy completes.</param>
        ...
        /// <param name="indexPointer">The native handle of a buffer belonging to the allocation to read.</param>
        /// <param name="keep">If <c>false</c>, the device memory is freed after the copy completes.</param>
        /// <returns>A task producing a host array with the copied data, or an empty array on failure.</returns>
        public async Task<T[]> PullDataAsync<T>(IntPtr indexPointer, bool keep = false) where T : unmanaged
        {
            return await this.PullDataAsync<T>(indexPointer, keep, null);
        }
        ...
        /// <param name="keep">If <c>false</c>, the device memory is freed after the copy completes.</param>
        /// <returns>A task producing a list with one host array per buffer, or an empty list on failure.</returns>
        public async Task<List<T[]>> PullChunksAsync<T>(IntPtr indexPointer, bool keep = false) where T : unmanaged
        {
            return await this.PullChunksAsync<T>(indexPointer, keep, null);
        }
