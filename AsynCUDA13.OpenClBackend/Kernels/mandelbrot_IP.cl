// Mandelbrot set fractal - in-place (IP, float I/O pointer). Linear dispatch: launch with
// global size = width*height (one work-item per pixel). The width/height/channels parameters
// are injected by the OpenCL image launcher; the remaining scalar arguments (zoom, offsetX,
// offsetY, iterCoeff, baseR, baseG, baseB) follow. Alpha (a 4th channel, if present) is forced
// fully opaque (255). This generator only writes, so image serves as the output buffer.
inline uchar clamp_channel(int v)
{
	return (uchar)(v < 0 ? 0 : (v > 255 ? 255 : v));
}

__kernel void mandelbrot_IP(
