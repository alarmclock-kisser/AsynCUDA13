// Julia set fractal - out-of-place (OOP, separate input/output pointers). Linear dispatch:
// launch with global size = width*height (one work-item per pixel). The width/height/channels
// parameters are injected by the OpenCL image launcher; the remaining scalar arguments (zoom,
// offsetX, offsetY, iterCoeff, juliaReal, juliaImag, baseR, baseG, baseB) follow. c =
// (juliaReal, juliaImag) is the fixed Julia constant. Alpha (a 4th channel, if present) is
// forced fully opaque (255). This generator writes into the dedicated output buffer;
// inputPixels is unused.
inline uchar clamp_channel(int v)
{
	return (uchar)(v < 0 ? 0 : (v > 255 ? 255 : v));
}

__kernel void JuliaFractal_OOP(
